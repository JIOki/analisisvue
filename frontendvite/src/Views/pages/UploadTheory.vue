...
const material = ref({
  ...
  owner: '',
  tags: '',
  category: 'MarcoTeorico', // puedes dejarlo fijo
  file: null
});
...
async function uploadMaterial() {
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log('📤 INICIANDO CARGA DE MATERIAL');
  console.log('═══════════════════════════════════════════════════════════');

  // Validación inicial
  if (!material.value.title || !material.value.file) {
    console.warn('⚠️ VALIDACIÓN FALLIDA: Título o archivo faltante');
    console.log('   - Título:', material.value.title || '❌ FALTANTE');
    console.log('   - Archivo:', material.value.file ? material.value.file.name : '❌ FALTANTE');
    console.log('═══════════════════════════════════════════════════════════');
    return;
  }

  console.log('✅ VALIDACIÓN EXITOSA');
  console.log('   - Título:', material.value.title);
  console.log('   - Autor:', material.value.author || 'No especificado');
  console.log('   - Propietario:', material.value.owner || 'No especificado');
  console.log('   - Etiquetas:', material.value.tags || 'No especificadas');
  
  // ✅ Garantizar que la categoría sea MarcoTeorico
  const categoryValue = 'MarcoTeorico';
  console.log('   - Categoría:', categoryValue);
  console.log('   - Archivo:', material.value.file.name);
  console.log('   - Tamaño archivo:', (material.value.file.size / 1024).toFixed(2), 'KB');
  console.log('   - User ID:', authStore.userId || authStore.user?.id || 'No disponible');

  // Activar estado de carga
  loading.value = true;
  success.value = false;
  console.log('🔄 Estado: CARGANDO (mostrando spinner)');
  ...
  formData.append('owner', material.value.owner);
  formData.append('tags', material.value.tags);
  formData.append('category', categoryValue); // ✅ Categoría hardcodeada para UploadTheory
  formData.append('user_id', authStore.userId || authStore.user?.id);
  formData.append('file', material.value.file);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('   - Código:', res.status);
    ...
    success.value = true;
    console.log('🔄 Estado: ÉXITO (mostrando mensaje de confirmación)');

    // ⏱️ GARANTIZAR mínimo 3 segundos de visualización del mensaje de éxito
    const MIN_DISPLAY_TIME = 3000; // 3 segundos mínimo
    const elapsedTime = Date.now() - startTime;
    const additionalDelay = Math.max(0, MIN_DISPLAY_TIME - elapsedTime);

    console.log('⏳ Tiempo transcurrido:', elapsedTime.toFixed(2), 'ms');
    console.log('⏳ Espera adicional para cumplir 3 segundos:', additionalDelay.toFixed(2), 'ms');
    console.log('⏳ Tiempo total de espera:', (elapsedTime + additionalDelay).toFixed(2), 'ms');

    // Usar Promise.resolve para asegurar que el setTimeout se ejecute correctamente
    await new Promise(resolve => setTimeout(resolve, additionalDelay));

    console.log('🔄 Estado: FINALIZANDO PROCESO');
    console.log('   - Limpiando formulario');
    console.log('   - Actualizando lista de materiales');
    console.log('   - Cerrando diálogo de carga');

    loading.value = false;
    resetForm();
    await fetchMaterials(); // ✅ recarga la tabla
    ...
    console.error('   - Tiempo transcurrido antes del error:', elapsedTime.toFixed(2), 'ms');
    console.error('═══════════════════════════════════════════════════════════');

    loading.value = false;
    console.log('🔄 Estado: ERROR (proceso detenido)');
  }
}
...
async function fetchMaterials() {
  try {
    ...
    // 🔍 Filtrar solo los que tienen categoría MarcoTeorico o son null (documentos subidos antes de la corrección)
    const marcoTeorico = data.filter(m => 
      m.category === "MarcoTeorico" || 
      m.category === "Marco Teórico" || 
      m.category === "Marco teórico" ||
      m.category === null ||
...
function seleccionarMarcoTeorico() {
  ...
  localStorage.setItem('marcoTeoricoIds', JSON.stringify(ids));
  router.push({ name: 'UploadCase' }); // o el nombre real de tu ruta
}
*/
...
<template #list="slotProps">
  ...
  <div class="flex flex-col h-full">
    <div class="card h-full flex flex-col">
      <h1 class="text-2xl font-bold mb-4">Biblioteca de Marco Teórico</h1>
      <Toolbar class="mb-4">
        <template #start>
      ...
      <Toolbar class="mb-6">
        <template #start>
          <Button label="Seleccionar marco teorico para analisis" icon="pi pi-check" severity="secondary"
            class="mr-2" 
            @click="seleccionarMarcoTeorico"
      ...
      <DataTable ref="dt" v-model:selection="selectedMaterials" :value="materials" dataKey="id" :paginator="true"
        ...
        :rowsPerPageOptions="[5, 10, 25]" class="h-full flex flex-col"
    ...
    <Dialog v-model:visible="materialDialog" :style="{ width: '600px' }" header="Nuevo material teórico" :modal="true"
  ...
  <Dialog v-model:visible="showEditDialog" :style="{ width: '600px' }" header="Editar material teórico" :modal="true"
    class="p-dialog-content">
    <div class="flex flex-col gap-6">
      <div>
        <label for="title" class="block font-bold mb-3">Título</label>
