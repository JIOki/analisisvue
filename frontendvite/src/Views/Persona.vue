  ...
  <div>
    ...
    <Dialog v-model:visible="productDialog" :style="{ width: '450px' }" header="Detalles del producto" :modal="true">
      <div class="flex flex-col gap-6">
        <div>
          ...
          <InputText id="title" v-model.trim="product.title" required autofocus :invalid="submitted && !product.title" />
          <small v-if="submitted && !product.title" class="text-red-500">El título es obligatorio.</small>
        </div>
        ...
        <div>
          ...
          <InputText id="owner" v-model="product.owner" />
        </div>

        <div>
          <label for="tags" class="block font-bold mb-3">Etiquetas</label>
          <InputText id="tags" v-model="product.tags" placeholder="teoría, fundamentos, educación" />
        </div>

        <div>
          <label for="rating" class="block font-bold mb-3">Reviews</label>
          <Rating id="rating" v-model="product.rating" />
        </div>
      </div>
    ...
    <Dialog v-model:visible="deleteProductDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
      <div class="flex items-center gap-4">
        ...
        <span v-if="product">¿Seguro que deseas eliminar <b>{{ product.title }}</b>?</span>
      </div>
      <template #footer>
    ...
    <Dialog v-model:visible="deleteProductsDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
      <div class="flex items-center gap-4">
        <i class="pi pi-exclamation-triangle text-3xl" />
        <span>¿Seguro que deseas eliminar los productos seleccionados?</span>
      </div>
      <template #footer>
