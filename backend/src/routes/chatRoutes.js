...
router.post('/chat/upload-documents', authenticateToken, upload.array('documents', 5), async (req, res) => {
  ...
  try {
    ...
    client.release();
    // ✅ LIMPIAR archivos temporales subidos al chat
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        if (file.path && fs.existsSync(file.path)) {
          fs.unlink(file.path, (err) => {
            if (err) console.error(`❌ Error al eliminar archivo temporal: ${file.path}`, err);
            else console.log(`🗑️ Archivo temporal eliminado: ${file.path}`);
          });
        }
      }
    }
